...
var (
	...
	// if the device replies with a mismatching header. This usually means the device is in browser mode.
	errLedgerReplyInvalidHeader = errors.New("ledger: invalid reply header")

	// errLedgerInvalidVersionReply is the error message returned by a Ledger version retrieval
	// when a response does arrive, but it does not contain the expected data.
	errLedgerInvalidVersionReply = errors.New("ledger: invalid version reply")
)
